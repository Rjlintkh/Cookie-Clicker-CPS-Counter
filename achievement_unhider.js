Game.registerMod("Achievement Unhider", {
	name: "Achievement Unhider",
	version: "1.0.0",
	GameVersion: "2.031",
	init: () => {
        Game.crate = function (me, context, forceClickStr, id) {
			...
			{
				if (context=='stats' && me.won==0 && me.pool!='normal' && false) return '';
        ...
        Game.crateTooltip = function (me, context) {
			...
			else if (me.type=='achievement')
			{
				...
				else if (me.pool=='dungeon') tags.push('Dungeon Achievement','#9e9e9e');
				else tags.push('Achievement',0);
				if (me.won>0) tags.push('Unlocked',0);
				else {tags.push('Locked',0);mysterious=0;}
